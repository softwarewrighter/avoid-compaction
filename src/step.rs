...
use crate::{StepConfig, StepStatus};
...
pub fn create_step(
    ...
    let config = StepConfig {
        ...
        created_at: crate::timestamp_iso(),
        completed_at: None,
...
pub fn transition_step(config: &mut StepConfig, to: StepStatus) -> Result<()> {
    ...
    if to == StepStatus::Completed {
        config.completed_at = Some(crate::timestamp_iso());
    }
...
/// Save a transcript file with timestamp in the saga dir root.
pub fn save_transcript(saga_dir: &Path, content: &str) -> Result<PathBuf> {
    let filename = format!("{}-transcript.txt", crate::timestamp());
    let path = saga_dir.join(&filename);
